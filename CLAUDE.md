...
# Test (138 unit + 42 integration as of 0.11.0)
cargo test
...
| `src/downloader/resolve.rs` | Pure URL-candidate generator. `candidate_urls(name, version, repo, &[targets])` produces 3 GitHub Release conventions × 2 archive formats × N target aliases. `current_targets()` returns the alias array per (arch, os) — macOS aarch64 gets 3, Linux x86_64 gets 4 — so HEAD probing covers `aarch64-apple-darwin` / `arm64-apple-darwin` / `darwin-arm64` / etc |
| `src/downloader/fetch.rs` | HTTP fetch. HEAD-probes candidates in order, first 2xx wins. `reqwest::bytes_stream()` GET writes to `TempDir/archive.{ext}` with cancel-check between chunks. Best-effort `.sha256` sidecar (404 tolerated; if present and mismatched → `Failed(ChecksumMismatch)`). Returns `FetchedArchive` owning the TempDir |
| `src/downloader/archive.rs` | Extract tar.gz / zip / raw bin into a fresh `TempDir`. `find_binary` does BFS depth-3 to handle both "binary at archive root" (mdbook) and "binary in versioned subdir" (ripgrep, cargo-deny) layouts. `ExtractResult` carries the TempDir for Drop cleanup |
| `src/downloader/install.rs` | Atomic install to `~/.cargo/bin`. Copy to `.cargo-fresh-{name}-{uuid}.tmp` → chmod 0o755 → fsync → `fs::rename`. Rename failure paths explicitly `remove_file` the tmp. Updates `.crates2.json` `installs` map via `package::crates2::write_install_record` (handles `package_name ≠ binary_name` case via `bins[]` lookup) |
| `src/downloader/mod.rs` | `download_and_install(client, spec, old_version, events_tx, cancel) -> Result<InstallOutcome, DownloaderError>` wires the 4 stages together with cancel checks at every async boundary. `InstallSpec { name, version, repo_url }` — caller (updater) is responsible for fetching `repo_url` via `crates_api::fetch_repo_url` |
| `src/ui/download_view.rs` | Crossterm region renderer for concurrent download progress (replaces indicatif MultiProgress). 10Hz repaint via tokio interval, Done/Failed scroll above the region via `println_above`. **Dormant in 0.11.0** — wired in 0.12.0 with the concurrent scheduler |
| `src/updater/mod.rs` | Builds cargo args per source via async `tokio::process::Command`, runs commands through per-package spinner, retry loop, dry-run short-circuit; `PbGuard` RAII guards spinner cleanup; `SlowGuard` aborts the 30s slow-warning watchdog; `verify_and_report_update` covers both primary and binstall→install fallback paths; `CommandSelector` makes the binstall→install fallback sticky so retries never revert to binstall |
...
- **`--format=json` JSON_MODE**: a global `AtomicBool` in `display::mod.rs`. All `status*` / `pb_status*` / `print_results` / `print_update_summary` short-circuit when set, and main emits one `JsonReport` line at the end. `schema_version = 1` is the 1.0 commitment — additive only. Schema is documented in `docs/json-schema.json` (Draft 2020-12) — when adding fields to `JsonReport`, update the schema file in the same commit. 0.10.3+ additions all under `schema_version=1`: `skipped[].reason_code` (enum), `version_check_errors[]` (errors that previously failed silently on `fresh[]`), `summary.selected`/`attempted`/`check_errors`. 0.10.4+ adds `updates_available[].binstall` (populated only when `--check-binstall` is set; `null` otherwise)
