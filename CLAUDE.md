...
# Test (165 unit + 55 integration as of 0.12.3)
cargo test
...
| `src/downloader/probe.rs` | `--check-prebuilt` implementation (replaced the old `cargo binstall --dry-run` probe in 0.11). Uses the **same** GitHub Releases API + HEAD-probe logic as the real update path, so the verdict matches what update would actually do. `probe_prebuilt(client, name, version)` returns the ternary `PrebuiltAvailability { Prebuilt / Source / Unknown }`; `annotate_updates` runs it **serially** across candidates (each call already saturates `Semaphore(16)` internally — concurrent probes would trip anonymous GitHub rate limits) and writes onto `PackageInfo.prebuilt`. Off by default |
...
| `src/downloader/mod.rs` | `download_and_install(client, spec, old_version, events_tx, cancel) -> Result<InstallOutcome, DownloaderError>` wires the stages with cancel checks at every async boundary. **API-first**: `try_api_winning_url` calls `github_api::fetch_release_assets` across 6 tag-name candidates, and on a hit returns a single-element candidate list (fetch then does 1 HEAD + stream GET); only when the API is unreachable / rate-limited / non-github does it fall back to `resolve::candidate_urls`'s full 360-URL enumeration. `InstallSpec { name, version, repo_url, bins }` — caller (updater) fills `repo_url` from `crates_api::fetch_repo_url` and `bins` from `crates2::lookup_bins` |
| `src/updater/mod.rs` | Per-package update orchestration via async `tokio::process::Command`, each through a row in the shared `MultiProgress`, with dry-run short-circuit. **Path policy** (`update_package`): crates.io + default features → self-hosted downloader first (`try_downloader_install`), falling through to `cargo install` on Unsupported/Failed; crates.io + custom features / git / path → `cargo install` directly. `build_args` no longer emits `binstall` — Crates source is `install --force <pkg> --version V`. `CommandSelector` now just holds the `cargo install` args (the old binstall-primary→install-fallback mode is gone); `cargo install` itself retries 3×. `UpdatePlan::new(names)` pre-registers all selected packages as right-aligned `pending` rows (rustup-style stacked list). Per-package state machine: `pending` → `resolving` → `[bar]` → `verifying` / `extracting` / `installing` → `installed X.XX MiB` (or `compiling from source` → `installed` for the cargo-install path). `finalize_installed/_failed/_aborted` swap the row to a static line via `pb.finish()`. `PbGuard` RAII is a panic-safety net. `SlowGuard` aborts the 30s slow-warning watchdog. Concurrency itself (`JoinSet` + owned-permit `Semaphore` for `--jobs`) lives in `main::run` / `run_one_update`, which sorts results back to input order |
...
Detailed item-by-item plan lives in `ROADMAP.md`. Status as of **v0.12.3**:
